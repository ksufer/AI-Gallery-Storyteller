...
export const parseImageFile = async (file: File): Promise<ImageMetadata> => {
    const textChunks = await extractPngTextChunks(file);
...
const extractPngTextChunks = async (file: File): Promise<Record<string, string>> => {
    ...
    let offset = 8;
    // We use Latin1 (ISO-8859-1) for parsing binary mixed content if needed, 
    // but text chunks are usually pure text. 
    ...
    while (offset < view.byteLength) {
        ...
        // Chunk Type (4 bytes)
        const type = new TextDecoder('ascii').decode(buffer.slice(offset + 4, offset + 8));
